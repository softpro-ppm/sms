...
@section('title', 'Add Staff - ' . $trainingPartner->name)
@section('page-title', 'Add Staff - ' . $trainingPartner->name)

@section('content')
<div class="max-w-2xl">
    <div class="mb-4">
        <a href="{{ route('admin.super.training-partners.show', $trainingPartner) }}" class="text-primary-600 hover:text-primary-900">
            <i class="fas fa-arrow-left mr-1"></i> Back to {{ $trainingPartner->name }}
        </a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form method="POST" action="{{ route('admin.super.training-partners.staff.store', $trainingPartner) }}" class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $trainingPartner->contact_name) }}" required
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $trainingPartner->contact_email) }}" required
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-gray-500">Minimum 8 characters. Share securely with the partner.</p>
                @error('password')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.super.training-partners.show', $trainingPartner) }}" class="px-5 py-2.5 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200">
                    Cancel
                </a>
            </div>
        </form>
